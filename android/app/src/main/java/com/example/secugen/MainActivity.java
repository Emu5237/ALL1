...
public class MainActivity extends  FlutterActivity {
    ...
    private static final String CHANNEL = "secugen";
    @Override
    public void configureFlutterEngine(@NonNull FlutterEngine flutterEngine) {
      ...
      new MethodChannel(flutterEngine.getDartExecutor().getBinaryMessenger(),CHANNEL)
                .setMethodCallHandler(
                        (call,result) ->{
                            if(call.method.equals("permission")) {
                            
                                // result.success("Hello Java");
                                result.success( initFingerPrintDevice());
                            }
                            if(call.method.equals("initialize")) {
                                    ...
                                    // result.success("Capture0");
                                    // mFingerPrintImage = new byte[mImageWidth * mImageHeight];
                                    try
                                {
                                             mFingerPrintImage = new byte[mImageWidth * mImageHeight];
                                                //  result.success(new String(mFingerPrintImage));
                                    sgfplib.GetImageEx(mFingerPrintImage, 10000, 70);
                                    // result.success("Capture2");
                                    Bitmap bpm =ImageConverter.toGrayscale(mFingerPrintImage, mImageWidth, mImageHeight);
                                     result.success(BitMapToString(bpm));
                                    //  result.success(bpm);
                                    // imgFingerPrint.setImageBitmap();
                                    result.success("Capture3");
                                }catch(Exception e){
                                    ...
                                    // result.error("PERMISSION_ERROR", null, null);
...
System.out.println("OnCreate");
   ...
   private boolean onViewReady() {
    ...
    private boolean initFingerPrintDevice() {
    ...
    private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_USB_PERMISSION.equals(action)) {
                ...
                btnTakeFingerPrint.setEnabled(false);
                Toast.makeText(MainActivity.this, "Fingerprint Device Removed", Toast.LENGTH_SHORT).show();
            }
        }
    };
}
